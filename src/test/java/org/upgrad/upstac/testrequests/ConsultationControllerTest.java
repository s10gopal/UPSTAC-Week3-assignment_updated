...
class ConsultationControllerTest {
  ...
  public void
    ...
    // Consultation using assignForConsultation() method
  ...
  public void
    ...
    // create the object. Pass the above created object as the parameter
    // Create another object of the TestRequest method and explicitly update the status of this
    // object
    // to be 'COMPLETED'. Make use of updateConsultation() method from labRequestController class
    ...
    TestRequest result =
        consultationController.updateConsultation(
            testRequest.getRequestId(), createConsultationRequest);

    // Use assertThat() methods to perform the following three comparisons
    //  1. the request ids of both the objects created should be same
    //  2. the status of the second object should be equal to 'COMPLETED'
    // 3. the suggestion of both the objects created should be same. Make use of getSuggestion()
    // method to get the results.

    assertThat(testRequest.getRequestId(), equalTo(result.getRequestId()));
    assertThat(result.getStatus(), equalTo(RequestStatus.COMPLETED));
    assertThat(
        createConsultationRequest.getSuggestion(), equalTo(result.consultation.getSuggestion()));
  }
  ...
  public void calling_updateConsultation_with_invalid_test_request_id_should_throw_exception() {
    ...
    // Create an object of ResponseStatusException . Use assertThrows() method and pass
    // updateConsultation() method
    // of consultationController with a negative long value as Id and the above created object as
    // second parameter
    ...
    ResponseStatusException result =
        assertThrows(
            ResponseStatusException.class,
            () -> {
              consultationController.updateConsultation(-1000001L, createConsultationRequest);
            });
  ...
  public void calling_updateConsultation_with_invalid_empty_status_should_throw_exception() {
    ...
    // Create an object of ResponseStatusException . Use assertThrows() method and pass
    // updateConsultation() method
    // of consultationController with request Id of the testRequest object and the above created
  ...
  public CreateConsultationRequest getCreateConsultationRequest(TestRequest testRequest) {
    ...
    // if the lab result test status is Positive, set the doctor suggestion as "HOME_QUARANTINE" and
    // comments accordingly
    // else if the lab result status is Negative, set the doctor suggestion as "NO_ISSUES" and
    // comments as "Ok"
